...
## Independent snippet slicing

`source2slice/snippet_slicer.py` provides an independent component that accepts a code snippet and a vulnerable line to produce interprocedural backward slices. This component expects the snippet to already be parsed into the Joern database (i.e., the Joern index and Neo4j server are prepared for the snippet path).

Example:

```
python source2slice/snippet_slicer.py --code-file /path/to/snippet.c --vuln-line 42 --test-id <folder_name>
```

If `--test-id` is omitted, the parent directory name of the snippet file is used.
